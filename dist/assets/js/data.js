/* Shared content. Set verified contact details here before accepting enquiries. */
window.CASPIAN = {
  name: "Security Caspian Shield Ltd",
  phone: "+44 7532 817131",
  whatsapp: "+44 7532 817131",
  email: "security@caspianshield.com",
  instagram: "https://www.instagram.com/security_caspian_shield/",
  services: [
    {
      id: "security-guard",
      name: "Security Guard",
      short: "A reassuring presence. A watchful eye.",
      description: "Professional guarding for people and premises.",
      heading: "A visible presence.\nLasting peace of mind.",
      intro:
        "A professional on-site presence to protect your premises and support daily operations.",
      image: "team.webp",
      icon: "shield",
      suitable:
        "Offices, retail premises, residential buildings and commercial properties",
      features: [
        [
          "On-site presence",
          "A clear security presence at the locations and times agreed with you.",
        ],
        [
          "Access control",
          "Support with visitor sign-in, deliveries and authorised entry.",
        ],
        [
          "Routine checks",
          "Patrol routes shaped around entrances, shared spaces and vulnerable areas.",
        ],
        [
          "Incident reporting",
          "Clear observations and escalation in line with your site procedures.",
        ],
      ],
      faqs: [
        [
          "Can guarding cover short-term requirements?",
          "Share your dates and hours. We’ll confirm coverage and availability.",
        ],
        [
          "What should I include in my enquiry?",
          "Include your location, property type, hours and main concerns.",
        ],
      ],
    },
    {
      id: "mobile-patrol",
      name: "Mobile Patrol",
      short: "Protection that keeps moving.",
      description: "Scheduled visits to protect your property.",
      heading: "Your property.\nOur watchful eye.",
      intro:
        "Regular security visits, scheduled around your property and operating hours.",
      image: "premises.webp",
      icon: "patrol",
      suitable:
        "Business parks, vacant properties, commercial premises and multiple sites",
      features: [
        [
          "Planned patrol visits",
          "Agree a visit pattern around your operating hours and identified concerns.",
        ],
        [
          "External inspections",
          "Checks of gates, doors, fencing and accessible perimeter areas.",
        ],
        [
          "Visible deterrence",
          "A recurring security presence to discourage unauthorised activity.",
        ],
        [
          "Clear escalation",
          "Report unusual activity and follow your agreed contact and response procedures.",
        ],
      ],
      faqs: [
        [
          "How often can patrols visit?",
          "Visits are agreed around your site and preferred schedule.",
        ],
        [
          "Can you cover more than one property?",
          "Share each location so we can assess a suitable patrol plan.",
        ],
      ],
    },
    {
      id: "door-supervisor",
      name: "Door Supervisor",
      short: "A warm welcome. A secure entrance.",
      description: "Safe, welcoming entry management.",
      heading: "First impressions.\nSecurity that lasts.",
      intro:
        "A professional welcome, controlled access and calm support at your entrance.",
      image: "team.webp",
      icon: "door",
      suitable:
        "Hospitality venues, private functions, entertainment venues and receptions",
      features: [
        [
          "Entry management",
          "Support guest lists, admissions and your venue’s entry policy.",
        ],
        [
          "Queue coordination",
          "Help keep entrances organised and pedestrian routes accessible.",
        ],
        [
          "Calm communication",
          "Handle concerns respectfully and escalate situations through agreed procedures.",
        ],
        [
          "Venue coordination",
          "Work with your management team around busy periods, arrivals and closing.",
        ],
      ],
      faqs: [
        [
          "Can I request cover for a single evening?",
          "Yes. Single-date and regular cover are subject to availability and an agreed brief.",
        ],
        [
          "How many supervisors does my venue need?",
          "Staffing depends on your capacity, layout, hours and risk assessment.",
        ],
      ],
    },
    {
      id: "event-security",
      name: "Event Security",
      short: "Your event. Everyone’s peace of mind.",
      description: "Security for a safe, well-run event.",
      heading: "A memorable event.\nFor all the right reasons.",
      intro:
        "Keep arrivals, access and departures organised with security tailored to your event.",
      image: "team.webp",
      icon: "event",
      suitable:
        "Corporate events, private celebrations, community events and live entertainment",
      features: [
        [
          "Pre-event planning",
          "Identify access points, key timings and the security needs of your event.",
        ],
        [
          "Guest access",
          "Support ticket checks, guest lists and controlled entry points.",
        ],
        [
          "Crowd movement",
          "Help guide arrivals and departures and keep key routes clear.",
        ],
        [
          "Organiser liaison",
          "Maintain clear communication with the event team throughout the agreed coverage.",
        ],
      ],
      faqs: [
        [
          "When should I enquire?",
          "Enquire early with your date, venue and expected attendance.",
        ],
        [
          "Can security work alongside my event staff?",
          "Yes. We agree coordination with your event and venue teams.",
        ],
      ],
    },
    {
      id: "construction-security",
      name: "Construction Site Security",
      short: "Protect your site. Keep work moving.",
      description: "Protect your site, equipment and access points.",
      heading: "Secure your site.\nProtect your progress.",
      intro:
        "Protect access points, equipment and materials throughout your project.",
      image: "site-officer.webp",
      icon: "building",
      suitable:
        "Active construction projects, refurbishment sites and development sites",
      features: [
        [
          "Controlled access",
          "Support the sign-in process for workers, contractors and deliveries.",
        ],
        [
          "Perimeter checks",
          "Observe gates, fencing and vulnerable access points.",
        ],
        [
          "Out-of-hours presence",
          "Discuss guarding or patrol requirements while your site is closed.",
        ],
        [
          "Site-focused reporting",
          "Record concerns and communicate with your nominated site contact.",
        ],
      ],
      faqs: [
        [
          "Can coverage change as the project develops?",
          "Yes. Review your coverage with us as the project changes.",
        ],
        [
          "Can patrols complement on-site guarding?",
          "Yes. Combine guarding and patrols around your site and hours.",
        ],
      ],
    },
  ],
};
